...
void allocateMemoryOnlySize(struct node **pList, int size)
{
    ...
    // Find the best fit node for the given size
    while (temp != NULL)
    {
        int diff = temp->end - temp->start - size;

        if (diff >= 0 && diff < minDiff)
        {
            // Found the best fit node
            minDiff = diff;
            bestFitNode = temp;
        }
        prev = temp;
    ...
    if (bestFitNode != NULL)
    {
        // If the block size exactly matches the required size, update the block
        if (minDiff == 0)
        {
        ...
        {
            // Update the start address of the current node
            bestFitNode->start += size;
        }
    }
    else
    {
        // No block of the given size found to allocate
